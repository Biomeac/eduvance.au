...
export default function UploadResource() {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (token) {
          ...
          await apiClient.getPendingRequests();
          setStaffUser({ authenticated: true });
          setStaffUsername(localStorage.getItem('staff_username') || '');
          setIsAuthReady(true);
        } else {
          setIsAuthReady(true);
        }
  ...
  useEffect(() => {
    const fetchExamSessions = async () => {
      ...
      try {
        const data = await apiClient.getExamSessions();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setStaffUsername(data.user.username);
      setMessage('Logged in successfully!');
      setMessageType('success');
      
      // Store token and username in localStorage
      localStorage.setItem('staff_token', data.session.access_token);
      localStorage.setItem('staff_username', data.user.username);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await apiClient.createResource({
        title,
        link,
        description,
  ...
  const handlePaperSubmit = async (e) => {
    ...
    try {
      const data = await apiClient.createPaper({
        subject_id: selectedSubjectId,
  ...
  const fetchPendingResources = async () => {
    try {
      ...
      setPendingResources(data.requests);
    } catch (error) {
  ...
  const rejectResource = async (id, retryCount = 0) => {
    ...
    try {
      await apiClient.rejectRequest(id, rejectionReason);
      ...
      if (retryCount < maxRetries) {
        ...
        if (shouldReload) {
          ...
          // Force refresh the pending resources
          setTimeout(() => {
            fetchPendingResources();
  ...
  const handleWatermark = async (resource) => {
    ...
    try {
      const res = await apiClient.watermarkPdf(resource.link);
  ...
  const handleSaveResourceChanges = async (id) => {
    ...
    try {
      await apiClient.updateRequest(id, updateData);
